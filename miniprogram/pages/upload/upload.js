...
function uploadModeEvidenceLine(mode = {}) {
  ...
  if (id === 'practice_recall' || id === 'review_recall') return '退出证据：隔天还能回忆第一步或错因。';
  ...
  const cards = (Array.isArray(split) ? split : []).map((item, index) => ({
    ...
    familyLine: item.id === 'ai_expression_layer'
      ...
      : item.id === 'local_release_gate'
        ? '家长可期待：隐私、放行、行为反馈和分享都先过规则闸门。'
        : '家长可期待：AI 给方法候选，本地证据决定是否继续。',
    boundaryLine: '边界：AI 不负责贴标签、放答案、发行为反馈或决定商业转化。'
  }));
  return {
...
Page({
    ...
    return {
      ...
      cards: [
        ...
        },
        {
          id: 'memory',
          title: '为什么不是白做',
          body: '关键错因会进入成长报告、AI私教和明天回访，变成后面还能用的学习资产。',
          tone: 'record'
        }
      ]
    };
  ...
  buildReportCta(decisionSource = {}, reportState = {}, options = {}) {
    ...
    const parentReportGenerationWorkflow = reportState.parentReportGenerationWorkflow
      || reportDraft.parentReportGenerationWorkflow
      || null;
    ...
    const uploadedMaterialDecisionDossier = reportState.uploadedMaterialDecisionDossier
      ...
      || null;
    const hasPracticeRecallReleaseEvidence = hasRealTaskReleaseEvidence;
    const servicePathwayAllowsPracticeRecall = !!(
      servicePathway
    ...
    const parentReportWorkflowView = {
      ...
      routes: {
        ...
        preview: `/pages/profile/profile?from=report_workflow_preview&panel=assessment&${query}`
      }
    };
    return {
  ...
  persistReportCtaToReportState(reportState = {}, cta = {}) {
    ...
    const nextState = Object.assign({}, reportState, {
      ...
      parentReportGenerationWorkflow: cta.parentReportGenerationWorkflow || null,
      parentReportWorkflowView: cta.parentReportWorkflowView || null,
  ...
  confirmReportParentConsent() {
    ...
    this.setData({
      lastReportCta: nextCta,
      parentReportWorkflowView: nextCta && nextCta.parentReportWorkflowView ? nextCta.parentReportWorkflowView : null
    });
  ...
  afterPrioritySaved(text, state, plan, mode) {
    ...
    this.setData({
      lastReportCta: latestReportCta,
      parentReportWorkflowView: latestReportCta && latestReportCta.parentReportWorkflowView ? latestReportCta.parentReportWorkflowView : null
    });
  ...
  openReportWorkflowRoute(event) {
    const dataset = event.currentTarget.dataset || {};
    const workflow = this.data.parentReportWorkflowView || {};
    const routes = workflow.routes || {};
    const key = dataset.workflow || 'preview';
    const route = routes[key] || {
      source: '/pages/upload/upload?from=report_workflow_source',
      modules: '/pages/tutor/tutor?from=report_workflow_modules',
      preview: '/pages/profile/profile?from=report_workflow_preview'
    }[key] || '/pages/profile/profile?from=report_workflow_preview';
    if (storage.recordSurfaceDepthAction) {
      storage.recordSurfaceDepthAction({
        surface: 'upload',
        dimensionId: 'upload_report_workflow_route',
        label: key,
        route,
        readiness: workflow.title || 'parent_report_workflow'
      });
    }
