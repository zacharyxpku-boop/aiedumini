...
Page({
  ...
  buildPlayableReviewTools(cards = []) {
    ...
    const fallback = {
      ...
      quiz: { title: '快闪问答', pitch: '极速反应知识挑战。快速判断，高频刺激极速过关。', duration: '约2分', readyCount: sourceCards.length, available: !!sourceCards.length },
      match: { title: '拼图配对', pitch: '核心概念连连看。拼合线索，建立知识关联网络。', duration: '约4分', readyCount: sourceCards.length, available: sourceCards.length >= 2 },
      snake: { title: '路线接龙', pitch: '逻辑推导解题连接。步步推导，连通思路直达终点。', duration: '约5分', readyCount: sourceCards.length, available: sourceCards.length >= 2 },
    };
    const display = {
