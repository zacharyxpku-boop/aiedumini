...
function buildProfileDossierDeliveryView(dossier = {}, servicePathway = {}) {
  ...
  return {
    ...
    boundaryLine: '边界：AI 不决定答案、过程反馈、标签、隐私和商业放行。'
    })),
...
function buildLearningReportSummary(reportState = {}, capabilityEvidenceLedger, subjectSkillDepth, curriculumSpine, visualSocraticMatrix, courseUnitMap, latestThinkingReceipt = null) {
  ...
  const publicK12PatternWorkbench = {
    ...
    releaseLine: '放行规则：没有孩子第一步、错因回放、次日回访和家长确认，不写长期画像，不发过程反馈，不生成可分享结论。',
    lanes: [
      {
        id: 'source',
        label: '公开资料',
  ...
  const personalizedParentReportExportPolicy = reportState.reportExportPolicy
    ...
    || null;
  const parentReportGenerationWorkflow = reportState.parentReportGenerationWorkflow
    || draft.parentReportGenerationWorkflow
    || (uploadedMaterialReportHandoff && uploadedMaterialReportHandoff.parentReportGenerationWorkflow)
    || null;
  const parentReportWorkflowView = reportState.parentReportWorkflowView
    || draft.parentReportWorkflowView
    || (uploadedMaterialReportHandoff && uploadedMaterialReportHandoff.parentReportWorkflowView)
    || null;
  const workflowSourceMap = parentReportGenerationWorkflow && parentReportGenerationWorkflow.sourceMap
    ? parentReportGenerationWorkflow.sourceMap
    : {};
  const workflowPromptEngineering = parentReportGenerationWorkflow && parentReportGenerationWorkflow.promptEngineering
    ? parentReportGenerationWorkflow.promptEngineering
    : {};
  const workflowImagePlan = parentReportGenerationWorkflow && parentReportGenerationWorkflow.imageGenerationPlan
    ? parentReportGenerationWorkflow.imageGenerationPlan
    : {};
  const workflowApiKey = parentReportGenerationWorkflow && parentReportGenerationWorkflow.apiKey
    ? parentReportGenerationWorkflow.apiKey
    : {};
  const workflowSteps = parentReportGenerationWorkflow && Array.isArray(parentReportGenerationWorkflow.pipeline)
    ? parentReportGenerationWorkflow.pipeline.slice(0, 4).map((item) => item.label || item.id).filter(Boolean)
    : [];
  const workflowModuleDecision = parentReportGenerationWorkflow && parentReportGenerationWorkflow.moduleDecision
    ? parentReportGenerationWorkflow.moduleDecision
    : {};
  const workflowSourceLine = workflowSourceMap.sourceCount
  ...
  return {
    ...
    actionProgressClass: reportHasDraft ? 'done' : 'locked',
    ...
    actionProgressAction: reportHasDraft ? '去查看' : '待解锁',
    ...
    parentReportWorkflowSteps: workflowSteps,
    ...
    realHomeworkPublicResourceTriageCards: realHomeworkCoverageMatrix && realHomeworkCoverageMatrix.publicResourceTriageBoard && Array.isArray(realHomeworkCoverageMatrix.publicResourceTriageBoard.resourceCards)
      ...
      : [],
    realHomeworkPublicResourceActionSeeds: realHomeworkCoverageMatrix && realHomeworkCoverageMatrix.publicResourceTriageBoard && Array.isArray(realHomeworkCoverageMatrix.publicResourceTriageBoard.sourceBackedActionSeeds)
      ? realHomeworkCoverageMatrix.publicResourceTriageBoard.sourceBackedActionSeeds
      : [],
    ...
    realHomeworkIntakeQueue: realHomeworkCoverageMatrix && Array.isArray(realHomeworkCoverageMatrix.publicK12HomeworkIntakeQueue)
      ...
      : [],
    realHomeworkIntakeRevisitDeck: realHomeworkCoverageMatrix && Array.isArray(realHomeworkCoverageMatrix.publicK12IntakeRevisitDeck)
      ? realHomeworkCoverageMatrix.publicK12IntakeRevisitDeck
      : [],
    ...
    realTrialRevisitRelayLine: realTrialRecoveryLoop ? realTrialRecoveryLoop.revisitRelayLine : '',
    realTrialRevisitRelayCards: realTrialRecoveryLoop && Array.isArray(realTrialRecoveryLoop.revisitRelayCards)
      ? realTrialRecoveryLoop.revisitRelayCards
      : [],
    ...
    realTrialRuleRetestReviewCards: realTrialRecoveryLoop && Array.isArray(realTrialRecoveryLoop.ruleRetestReviewCards)
      ...
      : [],
    realTrialRuleRetestRevisitCards: realTrialRecoveryLoop && Array.isArray(realTrialRecoveryLoop.ruleRetestCards)
      ? realTrialRecoveryLoop.ruleRetestCards
      : [],
    ...
    gameReturnEvidence,
    gameReturnEvidenceTitle: gameReturnEvidence ? '回访验证证据' : '',
    ...
    servicePathwayModeChoiceProtocol: servicePathway && servicePathway.modeChoiceProtocol ? servicePathway.modeChoiceProtocol : null,
    servicePathwayModeChoiceCards: servicePathway && servicePathway.modeChoiceProtocol && Array.isArray(servicePathway.modeChoiceProtocol.choiceCards) ? servicePathway.modeChoiceProtocol.choiceCards : [],
    servicePathwayModeChoiceGuardrails: servicePathway && servicePathway.modeChoiceProtocol && Array.isArray(servicePathway.modeChoiceProtocol.guardrails) ? servicePathway.modeChoiceProtocol.guardrails : [],
    servicePathwayModeChoicePositioning: servicePathway && servicePathway.modeChoiceProtocol ? servicePathway.modeChoiceProtocol.positioningLine || '' : '',
    servicePathwayModeChoiceReleaseGate: servicePathway && servicePathway.modeChoiceProtocol ? servicePathway.modeChoiceProtocol.releaseGate || '' : '',
    ...
    reportScoreReturnCard: reportDailyActionQueue && reportDailyActionQueue.scoreReportReturnCard ? reportDailyActionQueue.scoreReportReturnCard : null,
    reportScoreReturnDeck: reportDailyActionQueue && Array.isArray(reportDailyActionQueue.scoreReportReturnDeck) ? reportDailyActionQueue.scoreReportReturnDeck : [],
    reportScoreReturnLine: reportDailyActionQueue && reportDailyActionQueue.scoreReportReturnLine ? reportDailyActionQueue.scoreReportReturnLine : '',
    reportScoreReturnRoute: reportDailyActionQueue && reportDailyActionQueue.scoreReportReturnCard ? reportDailyActionQueue.scoreReportReturnCard.route : '',
    reportScoreReturnXpGate: reportDailyActionQueue && reportDailyActionQueue.scoreReportReturnCard ? reportDailyActionQueue.scoreReportReturnCard.xpGate : '',
    sevenDayActionBoard,
...
function buildFamilyDecisionActionBridge(input = {}) {
  ...
  const sharePath = `/pages/home/home?from=family_decision_bridge&challenge=review&mode=family_action&action=first_step_revisit&action_label=${encodeURIComponent('家庭决策行动桥')}&action_detail=${encodeURIComponent(memo.shareLine || sevenDay.today || '今晚只做一小步')}`;
  return {
    title: '家庭决策行动桥',
...
Page({
  data: {
    ...
    parentActionGuide: null,
    learningDepthMap: null,
    ...
    learningReportSummary: null,
    reportDailyActionQueue: null,
    learningReportInput: {
      mode: 'fast',
      sourceText: '',
      profileBasics: {
    ...
    learningReportRecognitionSummary: { mode: 'unavailable' },
  ...
  resolveReportJobCaseId(reportState = {}) {
    const state = reportState || {};
    const draft = state.reportDraft || {};
    const handoff = state.uploadReportHandoff || state.uploadedMaterialDecisionDossierHandoff || {};
    return state.reportJobCaseId
      || state.caseId
      || draft.caseId
      || draft.id
      || handoff.reportId
  ...
  refreshReportJobStatus(caseId = 'default') {
    if (!api.fetchReportJobStatus) return Promise.resolve(null);
    return api.fetchReportJobStatus(caseId).then((status) => {
      if (!status || !status.ok) {
        this.setData({
          reportJobStatus: status || { ok: false, caseId, status: 'report_job_status_missing' },
          'learningReportSummary.parentReportWorkflowImageLine': this.formatReportJobImageLine(status || { status: 'report_job_status_missing' }),
          'learningReportSummary.reportJobCaseId': (status && status.caseId) || caseId
        });
        return status || null;
      }
      this.setData({
        reportJobStatus: status,
        'learningReportSummary.parentReportWorkflowImageLine': this.formatReportJobImageLine(status),
        'learningReportSummary.reportJobCaseId': status.caseId || caseId
      });
      return status;
    }).catch(() => {
      this.setData({
        reportJobStatus: { ok: false, caseId, status: 'remote_status_unavailable' },
        'learningReportSummary.parentReportWorkflowImageLine': '本地报告预览可看，远程图稿状态未返回'
      });
      return null;
    });
  },
