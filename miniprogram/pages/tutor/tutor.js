...
function buildSocraticReasoningLine(receipt = {}) {
  const judge = receipt.socratic_prompt_quality_judge || receipt.socraticPromptQualityJudge || {};
  const contract = receipt.socratic_ai_local_boundary_contract || receipt.socraticAiLocalBoundaryContract || {};
  const effective = Array.isArray(judge.effectivePrompts) && judge.effectivePrompts.length
    ? judge.effectivePrompts[0]
    : null;
  const rows = Array.isArray(contract.runtimeDecisionRows) ? contract.runtimeDecisionRows : [];
  const localDecision = rows[0] && rows[0].localDecision ? rows[0].localDecision : '证据、提示层级和是否放行';
  const prompt = effective && effective.prompt ? effective.prompt : '先让孩子说第一步和依据';
  return `本地规则决定：${localDecision}；有效追问：${prompt}；AI 只改写成孩子听得懂的话。`;
}

function buildSocraticTrace(receipt = {}) {
  const judge = receipt.socratic_prompt_quality_judge || receipt.socraticPromptQualityJudge || {};
  const contract = receipt.socratic_ai_local_boundary_contract || receipt.socraticAiLocalBoundaryContract || {};
  const rows = Array.isArray(contract.runtimeDecisionRows) ? contract.runtimeDecisionRows : [];
  const effective = Array.isArray(judge.effectivePrompts) && judge.effectivePrompts.length
    ? judge.effectivePrompts[0]
    : null;
  const stop = Array.isArray(judge.stopConditions) && judge.stopConditions.length
    ? judge.stopConditions[0]
    : null;
  const blocked = Array.isArray(contract.aiMustNotDecide) && contract.aiMustNotDecide.length
    ? contract.aiMustNotDecide.slice(0, 2).join('、')
    : '答案、排名和是否放行';
  return {
    localLine: rows[0] && rows[0].localDecision ? rows[0].localDecision : '证据、提示层级、放行门槛由本地规则决定',
    promptLine: effective && effective.prompt ? effective.prompt : '只追问孩子的第一步和依据',
    boundaryLine: stop && (stop.rule || stop.prompt)
      ? (stop.rule || stop.prompt)
      : `AI 不决定：${blocked}`
  };
}

function buildSocraticPromptWorkflow(receipt = {}, result = {}, selected = {}, nextRoute = '') {
  const judge = receipt.socratic_prompt_quality_judge || receipt.socraticPromptQualityJudge || result.socratic_prompt_quality_judge || {};
  const contract = receipt.socratic_ai_local_boundary_contract || receipt.socraticAiLocalBoundaryContract || result.socratic_ai_local_boundary_contract || result.socraticAiLocalBoundaryContract || {};
  const effective = Array.isArray(judge.effectivePrompts) && judge.effectivePrompts.length
    ? judge.effectivePrompts[0]
    : null;
  const rows = Array.isArray(contract.runtimeDecisionRows) ? contract.runtimeDecisionRows : [];
  const aiMustNotDecide = Array.isArray(contract.aiMustNotDecide) ? contract.aiMustNotDecide : [];
  const aiMayRewrite = Array.isArray(contract.aiMayRewrite) ? contract.aiMayRewrite : [];
  const route = nextRoute || '/pages/review/review?from=socratic_prompt_workflow';
  return {
    id: 'socratic_prompt_workflow',
    turnId: receipt.turnId || '',
    selectedId: selected && selected.id ? selected.id : '',
    selectedPrompt: effective && effective.prompt ? effective.prompt : '先让孩子说第一步和依据',
    localDecision: rows[0] && rows[0].localDecision ? rows[0].localDecision : '本地规则判断证据、提示层级和放行边界',
    aiBoundary: {
      aiMustNotDecide: aiMustNotDecide.length ? aiMustNotDecide : ['original_question', 'full_answer', 'score', 'ranking'],
      aiMayRewrite: aiMayRewrite.length ? aiMayRewrite : ['child_friendly_question', 'one_step_hint', 'parent_safe_summary']
    },
    nextReviewRoute: route,
...
function findPublicK12IntakeCard(options = {}) {
  const from = String(options.from || '');
  const intakeId = String(options.intake_id || options.intakeId || options.revisit_id || options.revisitId || options.challenge_id || options.challengeId || '');
  const taskType = String(options.task_type || options.taskType || '');
  if (from !== 'public_k12_intake' && !intakeId && !taskType) return null;
  const deck = realHomeworkCoverage && realHomeworkCoverage.buildPublicK12IntakeRevisitDeck
    ? realHomeworkCoverage.buildPublicK12IntakeRevisitDeck({ limit: 21 })
    : [];
  if (!Array.isArray(deck) || !deck.length) return null;
  return deck.find((item) => item.id === intakeId)
    || deck.find((item) => item.taskType === taskType)
    || deck[0];
}

function buildPublicK12SelectedHomework(intakeCard = {}) {
  const taskType = intakeCard.taskType || 'unknown';
  const firstMove = intakeCard.observableFirstMove || intakeCard.firstStepPrompt || '先说出第一步，不要写完整答案。';
  const wrongCause = intakeCard.localTransform || intakeCard.gameUse || taskType;
  return {
    id: `public_k12_tutor_${intakeCard.id || taskType}`,
    text: intakeCard.prompt || intakeCard.title || `公开 K12 第一问：${intakeCard.subject || '当前学科'} · ${taskType}`,
    subject: intakeCard.subject || '',
    taskType,
    source: 'public_k12_intake',
    evidence: {
      source: 'public_k12_intake',
      public_k12_intake_id: intakeCard.id || '',
      source_id: intakeCard.sourceId || '',
      task_type: taskType,
      first_step_prompt: firstMove,
      wrong_cause: wrongCause,
      parent_check: intakeCard.shareHook || intakeCard.fallbackIfNoChildInput || '家长只问第一步和错因，不看完整答案。',
      misconception_tags: [
        { label: wrongCause, axis: taskType, hint: firstMove }
      ],
      blocked_fields: Array.isArray(intakeCard.blockedFields) ? intakeCard.blockedFields.slice() : [],
      share_safe_fields: Array.isArray(intakeCard.shareSafeFields) ? intakeCard.shareSafeFields.slice() : [],
      answer_boundary: intakeCard.answerBoundary || 'first_step_only_no_full_answer'
    },
    publicK12IntakeRevisit: {
      id: intakeCard.id || '',
      sourceId: intakeCard.sourceId || '',
      taskType,
      observableFirstMove: firstMove,
      fallbackIfNoChildInput: intakeCard.fallbackIfNoChildInput || '',
      receiverMustUseOwnMaterial: intakeCard.receiverMustUseOwnMaterial !== false,
      route: intakeCard.route || '',
      reviewRoute: intakeCard.reviewRoute || '',
      revisitRoute: intakeCard.revisitRoute || intakeCard.reviewRoute || '',
      returnRoute: intakeCard.revisitRoute || intakeCard.reviewRoute || intakeCard.route || ''
    }
  };
}

function publicK12TutorIntro(intakeCard = {}, selected = {}) {
  const subject = intakeCard.subject || selected.subject || '当前学科';
  const taskType = intakeCard.taskType || selected.taskType || 'unknown';
  const firstMove = intakeCard.observableFirstMove || selected.evidence && selected.evidence.first_step_prompt || '先说第一步。';
...
Page({
  ...
  onShow() {
    ...
    const publicK12IntakeCard = findPublicK12IntakeCard(routeOptions);
    const publicK12Selected = publicK12IntakeCard ? buildPublicK12SelectedHomework(publicK12IntakeCard) : null;
    ...
    if (publicK12Selected) {
      const sourceKey = `public_k12_intake:${publicK12IntakeCard.id || publicK12Selected.taskType}`;
      ...
      if (previousSource !== sourceKey) {
        storage.set(storage.KEYS.tutorMessages, [
          { role: 'assistant', text: publicK12TutorIntro(publicK12IntakeCard, publicK12Selected) }
        ]);
      }
    }
    ...
    const intro = publicK12IntakeCard
      ? publicK12TutorIntro(publicK12IntakeCard, selected)
      : entryTutorIntent
  ...
  appendAssistant(result, turnState = null) {
    ...
    if (storage.appendReviewEvent) {
      storage.appendReviewEvent({
        event: 'socratic_prompt_workflow',
        type: 'socratic_prompt_workflow',
        turnId: diagnosticReceipt.turnId,
        selectedId: promptWorkflow.selectedId,
        selectedPrompt: promptWorkflow.selectedPrompt,
        localDecision: promptWorkflow.localDecision,
        aiBoundary: promptWorkflow.aiBoundary,
        nextReviewRoute: promptWorkflow.nextReviewRoute,
        parentEvidenceRoute: promptWorkflow.parentEvidenceRoute,
        evidenceRequired: promptWorkflow.evidenceRequired,
    ...
    if (storage.recordUnifiedNextAction) {
      storage.recordUnifiedNextAction({
        source: 'socratic_prompt_workflow',
        sourceLabel: '苏格拉底点拨工作流',
        actionLabel: promptWorkflow.selectedPrompt,
        route: promptWorkflow.nextReviewRoute,
        readiness: 'prompt_reasoning_ready',
