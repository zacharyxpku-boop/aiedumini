...
function buildAiMaterialExecutionPath(raw = {}, sourceSchemaId = 'parent_report') {
  ...
  return {
    ...
    practiceRecallRoute: safeAiText(path.practiceRecallRoute || path.practice_recall_route, '/pages/review/review?from=ai_material_analysis'),
...
function classifyImportInput(text = '') {
  ...
  if (looksLikeMaterialExcerpt(value)) {
    ...
    return {
      ...
      shouldCreateFocus: false,
      sourceMeta,
      feedback: `收到${sourceMeta.label}。只处理你粘贴的摘录，先导入回看练习；不自动抓取链接、不解析文件、不直接给答案。`
    };
  }
