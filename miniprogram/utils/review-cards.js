...
function dailyRevisitTarget(summary) {
...
function rewardBoard(summary) {
  ...
  const revisitTarget = summary && summary.dailyRevisit ? summary.dailyRevisit : null;
  const rewards = [];
  if (revisitTarget) {
    rewards.push({
      id: `revisit:${revisitTarget.id}:${todayKey()}`,
      title: revisitTarget.title,
      progress: revisitTarget.progress,
      ready: !!revisitTarget.achieved,
      claimed: !!claimed[`revisit:${revisitTarget.id}:${todayKey()}`],
      xp: Number(revisitTarget.rewardXp || 0),
      lives: revisitTarget.id === 'leech_clear' ? 1 : 0,
      streakFreeze: revisitTarget.id === 'goal_run' ? 1 : 0,
      hint: revisitTarget.achieved ? '完成记录已就绪' : `完成后会写入 ${revisitTarget.rewardXp} 点练习记录`
    });
  }
...
function studySeason(summary) {
  ...
  const dailyRevisit = safe.dailyRevisit || {};
...
function practiceTemplateWorkshop(summary = {}, cards = []) {
  const safe = summary || {};
  ...
  const readyCount = Math.max(
    Number(safe.due || 0),
    safeCards.length,
    templates.reduce((sum, item) => sum + Number(item.total || 0), 0)
  );
...
function localProgressShareShell(summary) {
  const safe = summary || {};
  const season = safe.season || {};
  const fsrs = safe.fsrsCoach || {};
  const revisitTarget = safe.dailyRevisit || {};
  const missions = [
    {
      id: 'solo_focus',
...
function gameEconomy(summary) {
  ...
  const loop = safe.loop || {};
  const season = safe.season || {};
  const progressShare = safe.localProgressShare || {};
  const quests = [
    ...
    { id: 'quiz', title: '本机小测检查', reward: '+8 学习记录', trigger: '完成一次真实卡片小测', ready: !!progressShare },
...
function missionBoard(summary) {
  ...
  board.push({
    id: 'mission_leech',
    title: '至少处理 1 个顽固错因',
    current: summary.dailyRevisit && summary.dailyRevisit.id === 'leech_clear' && summary.dailyRevisit.achieved ? 1 : 0,
    target: 1,
    progress: summary.dailyRevisit && summary.dailyRevisit.id === 'leech_clear' ? summary.dailyRevisit.progress : 0,
    status: summary.dailyRevisit && summary.dailyRevisit.id === 'leech_clear' && summary.dailyRevisit.achieved ? 'done' : 'active'
  });
  board.push({
    id: 'mission_sync',
    title: '把本机记录接入账号连续性',
  ...
  const dimensions = [
    ...
    },
    {
      ...
      score: clampScore(
        ...
        + (safe.localProgressShare && safe.localProgressShare.missions && safe.localProgressShare.missions.length >= 3 ? 8 : 0)
        + (loop.streakFreeze >= 0 ? 10 : 0)
        + (loop.lives > 0 ? 10 : 0)
      ),
      gap: '真实多人协作和跨端记录需要登录与持久化服务后开放。'
    },
    {
      id: 'cloud_sync',
      label: '多端连续记录准备度',
...
function reviewSummary() {
  ...
  summary.dailyRevisit = dailyRevisitTarget(summary);
  ...
  summary.practiceTemplateWorkshop = practiceTemplateWorkshop(summary, cards);
  summary.localProgressShare = localProgressShareShell(summary);
