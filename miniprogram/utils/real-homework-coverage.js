...
function buildPublicK12AssetBoundaryAudit(options = {}) {
  ...
  return {
    ...
    antiFakeThicknessLine: '公开资料必须先改写为第一步/错因/短回访，禁止原题、标准答案、排名预测和拍照搜题承诺。'
  };
}
...
function buildRealHomeworkCoverageMatrix(options = {}) {
  ...
  return {
    ...
    homeworkIntakeQueueLine: `已把 ${PUBLIC_K12_HOMEWORK_INTAKE_QUEUE.length} 条公开K12作业形态排入采集队列：先改写为本地压力样本，再压测苏格拉底、报告、短回访和分享。`,
    intakeChallengeDeckLine: `已把 ${PUBLIC_K12_HOMEWORK_INTAKE_QUEUE.length} 条采集队列转成短回访卡：接收者必须用自己的材料说第一步，不能复制原题或答案。`,
