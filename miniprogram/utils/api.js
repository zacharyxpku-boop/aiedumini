...
function fetchReportJobStatus(caseId) {
  ...
  return new Promise((resolve, reject) => {
    wx.request({
      url: `${apiBase()}/api/report-job-status?case_id=${encodeURIComponent(safeCaseId)}`,
      method: 'GET',
      header: Object.assign({
        'content-type': 'application/json'
      }, clientHeader()),
      ...
      success(res) {
        ...
        if (res.statusCode === 404 && res.data && res.data.error === 'report_job_status_missing') {
          resolve(Object.assign({}, res.data, {
            ok: false,
            status: 'report_job_status_missing',
