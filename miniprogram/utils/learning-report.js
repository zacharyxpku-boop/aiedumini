...
function buildRecommendationPlan(parts, matrix) {
  ...
  const routeMap = {
    ...
    revisit: '/pages/review/review?from=learning_report',
...
function buildEvidenceBasedMethodologyGuide(methodCandidateCards = [], materialLanes = [], servicePathway = {}) {
  ...
  const cards = [
    ...
    },
    {
      ...
      reportLine: '报告会把它写成“明天回访哪张卡”，不是加题量或外部排名。',
...
function buildCommercialFamilySolutionBook(input = {}) {
  ...
  const executionPathMap = [
    ...
    },
    {
      ...
      recommended: recommendedModeId === 'practice_recall'
    },
    {
  ...
  return {
    ...
    pages: [
      ...
      },
      {
        ...
        body: methodCards.length ? methodCards.map((item) => item.method || item.label || item.title).filter(Boolean) : ['Socratic first-step validation', 'three-minute mini lesson only as rescue', 'practice recall only after real card evidence'],
    ...
    aiMaterialAnalysis: aiMaterialAnalysisContract && aiMaterialAnalysisContract.id ? {
      ...
      normalizedSolution: {
        ...
        executionPath: aiMaterialFallback.executionPath || {
          ...
          practiceRecallRoute: '/pages/review/review?from=solution_book_recall',
...
function buildGameReturnEvidence(input = {}) {
  ...
  return {
    ...
    source: 'revisit_review_local_loop',
    ...
    competitorClosureBenchmarks: [
      {
        ...
        productAnswer: '原点要把上传报告后的第一步直接送到AI私教，只追问第一步、错因和判断依据。',
...
function buildParentReportGenerationWorkflow(input = {}, context = {}) {
  const parts = context.parts || {};
  const parsed = context.parsed || {};
  const preview = context.personalizedParentReportPreview || null;
  const dossier = context.uploadedMaterialDecisionDossier || {};
  const sourceEvidenceLedger = context.sourceEvidenceLedger || {};
  const reportEvidenceReleaseGate = context.reportEvidenceReleaseGate || {};
  const servicePathway = context.servicePathway || {};
  const sources = Array.isArray(parts.reportSources) ? parts.reportSources : normalizeReportSources(input);
  const sourceText = [input.sourceText || '', input.scoreText || '']
    .concat(sources.map((source) => source.text || ''))
    .join('\n')
    .replace(/\s+/g, ' ')
    .trim();
  const imageCount = sources.reduce((sum, source) => sum + Number(source.imageCount || 0), 0);
  const apiKeyConfigured = !!(input.imageGenerationApiKeyConfigured || input.openaiApiKeyConfigured || input.serverImageGenerationReady);
  const materialTypes = Array.from(new Set(sources.map((source) => source.sourceSchemaId || source.type || 'manual_notes').filter(Boolean)));
  const evidenceGates = Array.from(new Set([]
    .concat(Array.isArray(reportEvidenceReleaseGate.requiredEvidence) ? reportEvidenceReleaseGate.requiredEvidence : [])
    .concat(Array.isArray(dossier.evidenceRequired) ? dossier.evidenceRequired : [])
    .concat(['parent_manual_confirmation', 'no_full_answer_or_private_fields'])
  ));
  const blockedFields = Array.from(new Set([
    'original_question',
    'full_answer',
    'score_ranking_publicity',
    'talent_label',
    'private_contact',
    'full_dialogue'
  ...
  const reportPrompt = [
    '把本地报告草案改写成家长能读懂的报告页。',
    '结构固定为：资料来源、孩子当前卡点、方法候选、今晚动作、明天回访、第 7 天验证、家长只问一句。',
    '不要展示原题照片、完整答案、完整对话、联系方式、公开排名或刺激性奖励。'
  ].join('\n');
  ...
  return {
    id: 'parent_report_generation_workflow',
    version: '2026-06-04.local-first-image-ready-v1',
    status: apiKeyConfigured ? 'server_image_generation_ready' : 'local_report_ready_image_waiting_for_api_key',
    apiKey: {
      requiredForLocalReport: false,
      requiredForHtmlPreview: false,
      requiredForImageRender: true,
      serverEnvCandidates: ['OPENAI_API_KEY'],
      currentStatus: apiKeyConfigured ? 'configured_on_server' : 'not_configured_do_not_block_local_flow'
    },
    sourceMap: {
      materialTypes,
      sourceCount: sources.length,
      imageCount,
      textExcerpt: sourceText.slice(0, 220),
      parsedScoreSubjects: Object.keys(parsed.parsedScores || {}),
      releaseState: parsed.requiresConfirmation ? 'parent_confirmation_required' : 'local_draft_ready'
    },
    pipeline: [
      { id: 'intake', label: '资料进入', module: 'miniprogram/pages/upload + import-intake', output: 'uploadIntakePacket + reportSources' },
      { id: 'local_reasoning', label: '本地推理', module: 'learning-report.buildLearningReportDraft', output: 'uploadedMaterialDecisionDossier + sourceEvidenceLedger' },
      { id: 'prompt_engineering', label: 'Prompt 编排', module: 'parent_report_generation_workflow', output: 'reasoningPrompt + reportPrompt + imageBriefPrompt' },
      { id: 'html_preview', label: '报告预览', module: 'personalized-report-template', output: 'personalizedParentReportPreview.html' },
      { id: 'action_loop', label: '行动闭环', module: 'tutor/review/profile routes', output: '今晚第一步 + 回访 + 家长确认' },
      { id: 'image_render', label: '报告图稿', module: 'server image generation API', output: 'report cover/long image after API key is configured' }
    ],
    promptEngineering: {
      reasoningPrompt,
      reportPrompt,
      imageBriefPrompt,
      modelInputSchema: ['reportSources', 'sourceEvidenceLedger', 'uploadedMaterialDecisionDossier', 'personalizedParentReportPreview.html', 'servicePathway'],
      blockedFields
    },
    imageGenerationPlan: {
      providerSlot: 'server_configured_image_generation',
      modelSlot: 'image_generation_model_configured_on_server',
      input: 'imageBriefPrompt + parentTopLine + safe report sections',
      output: 'report_visual_asset_url_or_temp_file',
      fallbackBeforeApiKey: preview && preview.html ? 'use_html_preview_and_parent_summary' : 'use_parent_summary_only',
      requiresServerSideKey: true
    },
    moduleDecision: {
      tutorRoute: '/pages/tutor/tutor?from=parent_report_workflow',
      reviewRoute: '/pages/review/review?from=parent_report_workflow',
      parentRoute: '/pages/profile/profile?from=parent_report_workflow',
      uploadRoute: '/pages/upload/upload?from=parent_report_workflow',
      recommendedMode: servicePathway && servicePathway.modeChoiceProtocol ? servicePathway.modeChoiceProtocol.recommendedModeId : 'socratic_first_step',
      localCodeOwns: ['evidence_gate', 'route_choice', 'privacy_fields', 'report_release', 'image_render_block_until_key'],
      aiMayRewrite: ['parent_friendly_explanation', 'socratic_prompt', 'image_brief_wording']
    },
    releaseGates: evidenceGates,
    parentReadableLine: apiKeyConfigured
      ? '资料已进入报告推理链路，服务端可继续生成报告图稿。'
      : '资料已进入报告推理链路；未配置 API Key 时先展示本地报告预览和行动闭环。'
  };
}
...
function buildLearningReportDraft(input = {}) {
  ...
  const parentReportGenerationWorkflow = buildParentReportGenerationWorkflow(input, {
    parts,
    parsed,
    uploadedMaterialDecisionDossier,
    sourceEvidenceLedger,
...
module.exports = {
  ...
  buildParentReportGenerationWorkflow,
