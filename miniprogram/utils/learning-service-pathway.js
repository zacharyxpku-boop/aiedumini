...
function pickModes(signals) {
  ...
  if (signals.hasRealTaskEvidence && (signals.needsMemory || signals.hasWrongQuestion)) ids.add('practice_recall');
  ...
  return MODE_CATALOG
    ...
    .map((mode, index) => Object.assign({}, mode, {
      priority: index + 1,
      localGate: mode.id === 'practice_recall'
        ? '必须先有真实错因或第一步证据'
...
function buildLockedModeCards(signals, modeRecommendations) {
  ...
  if (!signals.hasRealTaskEvidence) {
    lockedIds.push('three_minute_mini_lesson', 'practice_recall');
  }
  return MODE_CATALOG
...
function buildModeChoiceProtocol(signals, modeRecommendations, options = {}) {
  ...
  const choiceCards = modeRecommendations.map((mode) => {
    const isPracticeRecall = mode.id === 'practice_recall';
    const isMiniLesson = mode.id === 'three_minute_mini_lesson';
    const locked = (isPracticeRecall && !hasRealTaskEvidence) || (mode.id === 'wrong_question_repair_course' && !hasRealTaskEvidence);
  ...
  return {
    ...
    decisionOrder: ['socratic_private_tutor', 'three_minute_mini_lesson', 'practice_recall', 'parent_coaching', 'online_method_course', 'wrong_question_repair_course'],
...
function buildPersonalizedClosureBridge(signals = {}, modeRecommendations = [], validationPlan = [], partnerLedger = {}, postPilotRetentionLoop = {}, input = {}) {
  ...
  return {
    ...
    gameRetentionPlan: {
      ...
      openedPracticeRecall: openedModes.has('practice_recall'),
      ...
      comebackRoute: openedModes.has('practice_recall') ? '/pages/review/review?from=personalized_closure_bridge' : '/pages/review/review?from=personalized_closure_bridge'
    },
    scoreReportBridge: {
