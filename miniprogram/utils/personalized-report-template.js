...
function renderVisualToc(model = {}) {
  ...
  const rows = [
    ...
    ['F', '形成闭环', 'AI私教点拨、小课堂、回忆复习、家长复盘、30 天计划', '06']
  ];
  return `
...
function buildEvidenceCredibilityPages(model = {}) {
  ...
  return [
    ...
    renderReportPage('judgement-revision-path', '02.2 判断路径修正', '把天赋语言翻译成可验证路径', `
      ...
      <div class="analysis-block">
        <h3>本质判断</h3>
        <p>家长焦虑通常不是因为缺少建议，而是因为不知道建议为什么适合自己的孩子。本报告要把“孩子的潜能信号”与“方法选择”连起来，再让产品内的AI私教点拨、微型小课堂、回忆复习继续收证。</p>
      </div>
    `)
  ].join('');
}
  ...
  const dimensions = [
    {
      ...
      execution: ['AI私教先问“你听完这题，第一步想到什么”', '家长不要急着让孩子抄答案，先听 30 秒复述', '复述通过后再进入小黑板或书面过程'],
...
function buildProductLoopPages(model = {}) {
  ...
  const loopRows = [
    ['报告页', '家长看到 P0 科目、证据解释、今晚动作', '不展示原题照片和排名营销', '点击进入AI私教点拨'],
    ['AI私教点拨', '只追问题意、条件、第一步、错因', '不直接给完整答案', '生成错因卡'],
  ...
  return [
    ...
    renderReportPage('release-gate', '06.2 报告放行规则', '什么内容可以进入家长报告', `
      <div class="callout-grid">
        ...
        <div><b>必须可追踪</b><span>进入 review/focus 后要回收证据，更新下一版报告。</span></div>
      </div>
      ${renderKeyValueRows([
...
function buildMethodologyDeepPages(model = {}) {
  ...
  const intro = renderReportPage('methodology-evidence-base', '04.0 理论支撑', '为什么这些方法不是随便拼出来的', `
    <p class="lead">报告采用“学习科学 + AI私教点拨机制 + 家庭执行闭环”的三层依据。理论不是堆名词，而是为每个孩子选择更合适的入口、脚手架和验证方式。</p>
    ${renderDenseTable(
      ['理论/产品启发', '解决的问题', '落到报告里的写法', '产品内承接'],
      [
        ['苏格拉底追问 / Khanmigo', '孩子想直接要答案、过程不可见', '先问题意、条件、第一步和错因', 'AI私教对话'],
        ...
        ['Synthesis 式复杂任务', '只会单点知识，不会协作表达', '用规则变化和表达任务锻炼迁移', '游戏化回忆和练习'],
  ...
  return [
    renderReportPage('notice', '00 报告说明', '这份报告解决什么问题', `
      ...
      ${renderKeyValueRows([
        ...
        ['产品闭环', '报告 → AI私教点拨 → 小课堂/小黑板 → 回忆复习 → 家长复盘'],
    ...
    renderReportPage('executive', '01 核心结论', '家长先看这四件事', `
      ...
      <p>家长今晚只要确认：孩子能不能说出第一步。如果不能，产品进入苏格拉底 1v1 私教；如果连续卡住，再触发小课堂；如果能说清但容易忘，进入回忆复习和游戏化回访验证。这样做的逻辑不是硬套方法，而是把孩子已有的优势信号转成可验证的学习动作。</p>
    `),
