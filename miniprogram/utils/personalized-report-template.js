...
function renderVisualToc(model = {}) {
  ...
  const rows = [
    ...
    ['F', '形成闭环', 'AI 私教、小课堂、回忆复习、家长复盘、30 天计划', '06']
  ];
  return `
...
function buildEvidenceCredibilityPages(model = {}) {
  ...
  return [
    ...
    renderReportPage('judgement-revision-path', '02.2 判断路径修正', '把天赋语言翻译成可验证路径', `
      ...
      <div class="analysis-block">
        <h3>本质判断</h3>
        <p>家长焦虑通常不是因为缺少建议，而是因为不知道建议为什么适合自己的孩子。本报告要把“孩子的潜能信号”与“方法选择”连起来，再让产品内的 AI 私教、微型小课堂、回忆复习继续收证。</p>
      </div>
    `)
  ].join('');
}
  ...
  const dimensions = [
    {
      ...
      execution: ['AI 私教先问“你听完这题，第一步想到什么”', '家长不要急着让孩子抄答案，先听 30 秒复述', '复述通过后再进入小黑板或书面过程'],
...
function buildProductLoopPages(model = {}) {
  ...
  const loopRows = [
    ['报告页', '家长看到 P0 科目、证据解释、今晚动作', '不展示原题照片和排名营销', '点击进入 AI 私教'],
    ['AI 私教', '只追问题意、条件、第一步、错因', '不直接给完整答案', '生成错因卡'],
...
function buildMethodologyDeepPages(model = {}) {
  ...
  const intro = renderReportPage('methodology-evidence-base', '04.0 理论支撑', '为什么这些方法不是随便拼出来的', `
    <p class="lead">报告采用“学习科学 + AI 私教产品机制 + 家庭执行闭环”的三层依据。理论不是堆名词，而是为每个孩子选择更合适的入口、脚手架和验证方式。</p>
    ${renderDenseTable(
      ['理论/产品启发', '解决的问题', '落到报告里的写法', '产品内承接'],
      [
        ['苏格拉底追问 / Khanmigo', '孩子想直接要答案、过程不可见', '先问题意、条件、第一步和错因', 'AI 私教对话'],
  ...
  return [
    renderReportPage('notice', '00 报告说明', '这份报告解决什么问题', `
      ...
      ${renderKeyValueRows([
        ...
        ['产品闭环', '报告 → AI 私教 → 小课堂/小黑板 → 回忆复习 → 家长复盘'],
